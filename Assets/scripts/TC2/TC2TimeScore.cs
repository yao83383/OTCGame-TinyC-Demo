...
public class TC2TimeScore : MonoBehaviour
{
		...
		case 6:
		{
			...
			for (int i = 0; i < 10; i++)
			{
				for (int j = 0; j < 10; j++)
				{
					...
					{
						TC2Block block = world.blockCells[j, i];
						block.number = 0;
						//block.BlockSlotInst.RefreshSlotEdgesState();
						block.BlockSlotInst.RefreshSlotEdgeStateOnDestory();
