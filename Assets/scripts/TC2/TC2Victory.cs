...
public class TC2Victory : MonoBehaviour
{
	public TC2World world;

	public List<TC2Rocket> rockets = new List<TC2Rocket>();
	...
	public void StartVectory()
	{
		...
		if (world.freeBlocks.Count < 15)
		{
			do
			{
				TC2Block block = world.blockCells[Random.Range(1, 9), Random.Range(1, 9)];
				if (block.kind != 4)
				{
					block.kind = 0;
					block.DisAppear();
		...
		{
			TC2Block block2 = world.freeBlocks[Random.Range(0, world.freeBlocks.Count)];
			...
			rockets.Add(world.rockets[i].transform.GetChild(0).GetComponent<TC2Rocket>());
